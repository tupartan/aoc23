  ...
  def north(self, position):
    try:
      ...
      if(tile == START or tile == VERTICAL or tile == NORTH_WEST or tile == NORTH_EAST):
        ...
        if(not(northTile == START or northTile == VERTICAL or northTile == SOUTH_EAST or northTile == SOUTH_WEST)):
          north = False
    except: north = False
    return north
  def east(self, position):
    try:
      ...
      if(tile == START or tile == HORIZONTAL or tile == NORTH_EAST or tile == SOUTH_EAST):
        ...
        if(not(eastTile == START or eastTile == HORIZONTAL or eastTile == NORTH_WEST or eastTile == SOUTH_WEST)):
          east = False
    except: east = False
    return east
  def south(self, position):
    try:
      ...
      if(tile == START or tile == VERTICAL or tile == SOUTH_EAST or tile == SOUTH_WEST):
        ...
        if(not(southTile == START or southTile == VERTICAL or southTile == NORTH_WEST or southTile == NORTH_EAST)):
          south = False
    except: south = False
    return south
  def west(self, position):
    try:
      ...
      if(tile == START or tile == HORIZONTAL or tile == NORTH_WEST or tile == SOUTH_WEST):
        ...
        if(not(westTile == START or westTile == HORIZONTAL or westTile == NORTH_EAST or westTile == SOUTH_EAST)):
          west = False
    except: west = False
    return west
...
class Route:
  ...
  def getTile(self):
    return self.map.map[self.position[0]][self.position[1]]
  def takeStep(self):
...
route = Route(start, firstSteps[0], m)

while not route.getTile().__eq__("S"):
  print(f'{route.position} {route.getTile()}')
  route.takeStep()
  if(route.steps > 50): break
